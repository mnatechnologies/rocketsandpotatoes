...
ADD COLUMN IF NOT EXISTS certification_validated_by UUID REFERENCES staff_users(id),
ADD COLUMN IF NOT EXISTS certification_validated_at TIMESTAMPTZ;

-- Add review status if not exists
DO $$
BEGIN
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
        ...
        ALTER TABLE customer_documents
        ADD COLUMN reviewed_by UUID REFERENCES staff_users(id);
    END IF;
END $$;
